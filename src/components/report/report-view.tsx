...
import { downloadReportHtml } from "@/lib/report/report-download";
...
export function ReportView() {
  ...
  const mapUrl = gameCode ? `/map/${gameCode}` : "/map";
  ...
  const shareUrl = gameCode ? `/report/${gameCode}` : window.location.href;

  function handleDownload() {
    const session = getSession();
    const events = (() => {
      try {
        return localStorage.getItem("tenun-events")
          ? (JSON.parse(localStorage.getItem("tenun-events") || "[]") as SessionEvent[])
          : [];
      } catch {
      ...
      <div className="mx-auto w-full max-w-[1200px] px-6 lg:px-[80px] pb-16">
        ...
        <div className="flex flex-col gap-4 pt-8 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            {gameCode && (
              <span className="mt-2 inline-block rounded-full border border-[#FFB319] bg-[#0F3943] px-3 py-1 font-outfit text-[11px] font-bold text-[#FFB319]">
                KODE GAME: {gameCode}
              </span>
            )}
          </div>
