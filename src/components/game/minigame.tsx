...
export function Minigame({ game, onProgress, onComplete, onWrong, resumeKey = "default" }: Props) {
  ...
  // Live Demo: presenter-driven — TIDAK ada auto-play. Setiap klik tombol
  // demo (sinyal skip) memunculkan modal asesmen & menandai babak selesai.
  // Babak berikutnya hanya lanjut saat presenter mengklik lagi.
  useEffect(() => {
    if (demo && skip > 0) {
      progressRef.current(); // buka modal asesmen agar bisa dijelaskan presenter
      completeRef.current(); // tandai selesai (menunggu modal di-auto-jawab)
    }
  }, [demo, skip]);
